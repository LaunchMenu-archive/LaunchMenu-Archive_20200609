...
class IPC {
    ...
    static send(type, data, dest = "*", source = 0) {
        var encodedData = _extendedJSON2.default.encode(data);
        if (this._isRenderer()) {
            ...
            if (dest == "*") {
                ...
                dest = [];
                for (var i in windows) dest.push(i);
            ...
            dest.forEach(id => {
                if (id == 0) {
                    ...
                    if (window) window.webContents.send("IPC.recieve", { type: type, sourceID: source, data: encodedData });
                }
            });
        }
    }
    ...
    static __setup() {
        ...
        if (this._isRenderer()) {
            ...
            _electron.ipcRenderer.on("IPC.recieve", (event, arg) => {
                this.__emitEvent(arg.type, { sourceID: arg.sourceID, data: _extendedJSON2.default.decode(arg.data) });
            });
        } else {
            ...
            _electron.ipcMain.on("IPC.forward", (event, arg) => {
                this.send(arg.type, _extendedJSON2.default.decode(arg.data), arg.dest, arg.sourceID);
            });
        }
    }
}
